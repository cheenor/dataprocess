# -*- coding: utf-8 -*-
"""
Created on Sat Mar 07 08:30:11 2015
...
fname='ERA40-monthy1979-2001.nc'
datname='ERA40'
rgnm=[]
slon=[]
elon=[]
...
for ix in range(0,nx):
    for iy in range(0,ny):
        ...
        for i in range(0,nt-4,4):
            ...
            for j in range(0,4):
                ij=im*4+j
                tmp1=tmp1+sho[ij,iy,ix]
                tmp2=tmp2+lho[ij,iy,ix]
            sho_mon[iy,ix,im]=tmp1/4.
            lho_mon[iy,ix,im]=tmp2/4.
            im+=1
...
for iv in range(0,nvar):
    fpath=dirout+rgnm[iv]+datname+"_lhsh_1979-2001.txt"
    f=open(fpath,'w')
    itme="%s "%'Year'
    f.write(itme)
    itme="%s "%'DJF_SH'
                ...
                if lon[ix]>(slon[iv]+2.5) and lon[ix]<(elon[iv]-2.5):
                    for iy in range(0,ny):
                        if lat[iy]>slat[iv] and lat[iy]<elat[iv]:
                            ...
                            itmp+=1.
            tscale=6.*3600.
            seash="%f "%(-tmp1/itmp/tscale)
            sealh="%f "%(-tmp2/itmp/tscale)
            f.write(seash)
            f.write(sealh)
        f.write('\n')
    f.close()  